...
Prototyped in `docs/quick-add-prototype.html` (2026-08-06), then shipped in production code 2026-08-18: `src/components/ui/ChipPicker.vue` (favorite chips + borderless "More" overflow, reused for both Category and Payment method), `src/components/ui/DetailRow.vue` (the collapsed-optional-field pattern, reused for Date/Platform/Supplier/Notes), and `src/components/transactions/TransactionForm.vue` (the full Quick Add/Edit rewrite — both screens share this one component). Category and Payment method labels use the lighter field-label style, not the bold-caps section-header style — decided 2026-08-06 (C.12) after comparing both live in the prototype. **Administration retrofit done 2026-08-27**: `ConfigItemList.vue` (shared by Properties/Platforms/Categories/Payment methods/Suppliers), `AdministrationView.vue`, `CurrenciesView.vue`, and `UsersView.vue` all moved from bordered `neutral-200` boxes to the borderless `shadow-sm` language — inputs, cards, list wrappers, and the error/empty/skeleton states all now match `RoomsView.vue`/`RecurringPaymentsView.vue`. Two things deliberately left out of this pass: (1) `UsersView.vue`'s two role `<select>` elements kept native OS chrome rather than becoming custom borderless popovers — technically still owed per the "no native `<select>` chrome" rule above, but treated as a separate interaction-pattern change rather than bundled into a styling-only pass; (2) the same old bordered style also still exists outside Administration proper, in `AccountView.vue`, `SignInView.vue`, `TransactionDetailView.vue`, `EditTransactionView.vue`'s not-found state, and `ConfirmDialog.vue`'s cancel button — noted as related debt for a future pass, not touched here since this round was scoped to Administration specifically.
...
- **Skeleton loading:** `animate-pulse` blocks in `neutral-100` matching the shape of real content — never fake numbers or placeholder amounts. (Updated 2026-08-27 to match what's actually shipped; static `neutral-200` blocks were the original spec but every current screen uses the animated, lighter-tint version.)
