...
| 2026-08-02 | Revised the 2026-07-21 "no currency conversion" decision: Dashboard/Reports now also show one clearly-labeled *approximate* combined total, built from an administrator-maintained reference rate (Currencies admin, migration 0010) and computed only at report display time. Real per-currency totals still never blend. | With three currencies now genuinely in use, Jalie couldn't get an overall profit/loss sense from separate totals alone. Considered locking a rate in per-transaction (more accurate, but adds a field to the fastest/most-used screen) and a live FX API (most accurate, but a new external dependency) — chose the report-time-only approximation as the cheapest option that actually answers the question, with an explicit upgrade path noted in `06-development-roadmap.md` if accounting-grade accuracy is ever needed. |
