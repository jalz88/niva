...
**Language (2026-08-26):** the whole module (Hub, Today/Today's schedule, Rooms, Staff) is now bilingual — English and Sinhala, via `vue-i18n` (`src/i18n/`). Scope decided with Jalie: only Housekeeping/Staff is translated, since the caretaker and the manager reviewing her work are the ones who actually need it; Money/Administration stay English-only. An EN/සිං toggle sits in the Hub header (admin/manager) and in the Today kiosk header (staff) — one shared client-side preference (`localStorage`, not per-account), so picking a language in one place carries into the rest of the module. Sinhala strings are a first pass, not yet reviewed by a native speaker beyond the original prototype's validated Today-screen text — worth a read-through before leaning on it operationally.

**Room/task names (2026-08-26):** the language toggle only ever covers fixed UI chrome — a room's or task's own name is free text an admin typed in, so there's nothing to auto-translate. Jalie flagged that room and task names ("Coconut | CBR", "Change linen and towels") stayed in English even with Sinhala active; fix was an optional second name field rather than a translation service. Add/edit room and add/edit task sheets each gained an optional "Name (Sinhala)" / "Task (Sinhala)" field below the primary one (migration 0014, `rooms.name_si` / `sop_tasks.name_si`). When set, it's shown instead of the primary name anywhere the module renders in Sinhala (Today's room cards and checklist sheet, Rooms list and detail, Rooms checklist) via a shared `localizedName(name, nameSi)` helper (`useLocale()`); left blank, the primary name is shown regardless of language — always safe, never a blank field.

**Daily task flexibility — "Model A" (decided 2026-08-26 with Jalie and Maria over `docs/housekeeping-daily-task-flexibility-mockup.html`):** Maria's ask, relayed by voice note, was that the automatic checklist felt too rigid — not every task needs doing every day, and she wanted a way to hold it against the booking calendar. Two models were mocked side by side: a lightweight "skip/add override" where the automatic cadence stays the default and a manager only steps in when something's actually different, versus a heavier "daily confirm required" where nothing reaches the caretaker until a manager reviews and confirms it, closer to Recurring Payments' reminder-and-confirm shape. Model A (skip/add override) was chosen — no daily manager chore for a normal day. In the room checklist sheet, an administrator/manager gets a small "Skip today" action per task (excludes it from that room's progress total, shows it struck through with an "Undo skip" affordance) and an "Add a task for today" field at the bottom (creates a real task visible only today — migration 0015's `once` cadence — ticked off the same way as any other task, then gone from the live checklist the next day without lingering as a stale item). Neither control is shown to a caretaker (staff) account; she only sees the resulting list. The "hold it against the booking calendar" half of the request (shrinking a stayover room's list automatically) was left for a later round — not decided yet.

- **Hub**: three rows — "Today's schedule," "Rooms," "Staff" — same list-row pattern as Administration's landing screen.
- **Today / Today's schedule**: one shared screen, two audiences. A caretaker's own Today (`housekeeping-today`, any authenticated member, no nav chrome around it) shows only the rooms round-robin assignment (or a manual override) puts on that person today, and — since it's the only screen a kiosk account ever sees — carries its own thin header with the NIVA mark, the EN/සිං language toggle, and a **Sign out** button (2026-08-25/26, was in the original prototype's header but missed the first build). Today's schedule (`housekeeping-schedule`, administrator/manager) shows every room and adds a reassign chip-picker per card. Both show: a day-summary progress bar at the top (rooms fully done / total), then one card per room (progress bar, done/total tasks, last-update line, and — for a booking-linked room with an active `room_bookings` row today, 2026-08-25 — a "Checkout today" or "Stayover" badge; unlinked and unbooked rooms show no badge). The badge is context, not a filter: every room with something due still appears, checkout and stayover alike, since a stayover still needs its (lighter) tasks done. **Room order (2026-08-26):** checkout-today rooms lead the list, then stayovers, then everything else — alphabetical within each group — so the room that needs to be ready before the next guest arrives isn't buried wherever its name happens to sort. Tapping a room opens its checklist sheet — tasks are grouped by cadence, daily first then weekly/monthly/quarterly (2026-08-26, so the "do this every day no matter what" tasks aren't lost among less-frequent ones a caretaker only glances at occasionally) — tick/untick each task (unticking someone else's completed task asks for confirmation first, and is blocked outright for anyone but the completer or an administrator/manager, matching RLS), and once every task is done, a "Mark inspected" action for administrator/manager (a "Pending inspection" pill for anyone else) — inspection is optional, a spot-check, not a gate on anything else.
- **Task cadence, in practice**: a weekly task assigned to (say) Tuesday becomes "due" the moment Tuesday starts and stays on the checklist as not-done every day through the following Monday — it doesn't quietly disappear if she's busy on Tuesday itself. Once the next Tuesday arrives, the occurrence rolls forward to the new date; a Tuesday that passed without being ticked doesn't stack up as a separate lingering item, but it does count toward "overdue" on `housekeeping_attention_rooms` (Dashboard/Reports) for as long as it's the current, unticked occurrence. Monthly/quarterly cadences work the same way, just on a longer cycle (`sop_task_current_due_on`, migration 0012).
...
- Room assignment for the day is computed, not stored — active Housekeepers not marked off that day, distributed round-robin by creation order across the rooms with anything due; only a manual reassignment persists a row (`room_assignments`). The assigned name shown per room (Today's schedule) resolves `workforce_members.id` directly — a different id space from the completedBy/inspectedBy name resolution above, which chains through `auth.users.id`. Both silently fell back to "Someone" when fed the wrong id space; fixed as two separate lookups, 2026-08-26.
...
- Overdue cards additionally show a **Mark paid** button. A card due *today* also gets the button — it's payable now, same as overdue, it just isn't late yet, so it stays under the Upcoming heading with neutral (not red) styling (2026-08-26 fix). Anything due after today doesn't get the button.
