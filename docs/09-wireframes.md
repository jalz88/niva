...
- **Rooms**: list of rooms with type and booking-linked badge; tapping opens a detail view (booking-sync status card when linked to bookings, plus the room's cleaning checklist — SOP tasks with a cadence badge). Add/edit room sheet: name, type (chip picker), "Linked to bookings" toggle revealing a calendar export URL field when on. Add/edit task sheet: name, cadence (chip picker: daily/weekly/monthly/quarterly), matching weekday chips (weekly) or day-of-month field (monthly/quarterly). Both use archive, not hard delete.
