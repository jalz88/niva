...
Built: `supabase/migrations/0011_recurring_payments.sql` (table, integrity trigger, `mark_recurring_payment_paid()` RPC, RLS), `src/composables/useRecurringPayments.ts`, `src/lib/recurringPayments.ts` (cadence/due-date math, shared between the form and the Dashboard attention strip), `src/views/RecurringPaymentsView.vue`, a nav entry in `MoreSheet`/`AppShell.vue`, and a new "due soon" attention-strip item (`buildDuePaymentItems()` in `src/lib/attentionStrip.ts`) — overdue or due within 3 days, manager/administrator only, linking to the screen. Lint, typecheck, and build all clean.
