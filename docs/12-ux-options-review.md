...
| B2. Recurring bills | Option A. Renamed **Recurring payments** and shipped 2026-08-19 — see Part 2. |
...
**Shipped 2026-08-19, renamed Recurring payments.** Jalie brought a second real use case mid-build — Maria (the housekeeper) is paid weekly by standing bank order, with occasional overtime for extra days worked — which is the same "reminder to log a scheduled expense" shape as a bill, so the feature covers both under one name rather than building two. Designed collaboratively via an interactive HTML prototype (`docs/recurring-bills-prototype.html`, kept under its original filename) before any real code — a switcher let Jalie compare "confirm first" vs "instant, no confirm" for Mark paid, and "name + category" vs "category only" for telling payments apart, directly in the browser rather than from a description. Decisions locked in from that session:

- **Confirm first** for Mark paid (not instant) — a small sheet pre-filled with the saved amount, today's date, and an optional notes field, all editable. This is also how overtime is handled for now: bump the amount, note "2 extra days worked." No separate overtime concept in the data model — that's deferred until a Housekeeping calendar exists to auto-generate the figure (see `06-development-roadmap.md` item 5).
- **Keep the name field** — a short free-text label ("Wifi", "Maria — wages") separate from Category, since two payments can share a category (two rental units' electricity, or two staff members' wages) and would otherwise be indistinguishable.
- **No income variant** — recurring payments are always expenses.
- **Weekly cadence added** alongside monthly (day-of-month), needed for Maria's Sunday payments — a payment definition is exactly one or the other, chosen via a Monthly/Weekly toggle in the Add/Edit form.
- **Manager/administrator only**, not just visible-but-read-only — staff and viewer get no RLS policy on the table at all (migration 0011) and no nav entry in the More sheet, matching Jalie's explicit ask.
- **Edit and delete**, raised as a gap in the first prototype pass — tapping a payment card (anywhere except its Mark-paid button) opens it prefilled for editing, with a "Delete this payment" action below Save that opens the same confirm-dialog pattern already used for transaction delete. Deleting only stops future reminders; transactions already logged from it are untouched (`recurring_payment_id` is `ON DELETE SET NULL`).

Built: `supabase/migrations/20260819095201_recurring_payments.sql` (table, integrity trigger, `mark_recurring_payment_paid()` RPC, RLS), `src/composables/useRecurringPayments.ts`, `src/lib/recurringPayments.ts` (cadence/due-date math, shared between the form and the Dashboard attention strip), `src/views/RecurringPaymentsView.vue`, a nav entry in `MoreSheet`/`AppShell.vue`, and a new "due soon" attention-strip item (`buildDuePaymentItems()` in `src/lib/attentionStrip.ts`) — overdue or due within 3 days, manager/administrator only, linking to the screen. Lint, typecheck, and build all clean.
