...
1. ~~Export of Reports as CSV or print/PDF.~~ Done 2026-08-02 — see `src/lib/reportCsv.ts` and ReportsView's "Print / Save as PDF" (browser-native print, no PDF library). Exporting a filtered *transaction list* is still open — UX decided 2026-08-06: same CSV pattern, triggered from the Transactions list itself using whatever filters are currently applied, with a confirm step first if the filtered result exceeds 100 transactions. See `12-ux-options-review.md` Part B.1 and C.4.
2. Recurring bills and due-date reminders. UX decided 2026-08-06: a dedicated list, not folded into Transactions. See `12-ux-options-review.md` Part B.2.
...
7. In-app dashboard notifications — decided 2026-08-06 as a dashboard attention strip, computed live from existing data (no notification records, no read state). See `12-ux-options-review.md` Part B.4 and C.5.
