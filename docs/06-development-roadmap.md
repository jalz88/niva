...
5. ~~Housekeeping (rooms, cleaning checklists, staff roster, and the work calendar).~~ **Done 2026-08-24.** Rooms with cadence-based SOP checklists (daily/weekly/monthly/quarterly, `sop_task_current_due_on()` computing each occurrence), an append-only completion log (`sop_task_completions`) enabling real on-time/late trend data (not just a boolean done flag), optional per-room daily inspection (a spot-check, not a gate), a Staff roster with a work calendar (explicit days-off, no recurring pattern, no hours/timesheet — matches the 2026-08-06 "no micromanagement" decision) and an optional link to a recurring payment for pay, and computed (not stored) round-robin room assignment with manual override. The `staff` role was redefined for this (see `05-information-architecture.md`) — no separate role was added. Screen access (`visible_areas`) went from a documented model (2026-08-04) to an actual per-screen toggle sheet in Administration → Users. See `07-domain-model-and-schema.md` §3/§10/§11, migration `0012`, and `docs/housekeeping-in-app-prototype.html` for the prototype this was built from. Assets, maintenance, and inventory remain out of scope — speculative and far away. **Overtime, deferred 2026-08-19:** Recurring payments (item 2) deliberately doesn't model overtime as its own concept — a housekeeper's extra-days week is just an edited amount plus a free-text note at Mark-paid time. Auto-generating the overtime amount from the housekeeping work calendar (so the reminder shows up pre-calculated rather than needing a manual bump) is real future scope now that the calendar exists, but wasn't part of this build.
